  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        const {
          error
        } = await signUp(identifier, password, selectedRole, username);
          ...
          <Card className="p-8 bg-gradient-card shadow-medium">
            ...
            {isSignUp && <div className="mb-6">
                ...
                <div className="grid gap-3">
                  {roles.map(role => <RoleCard key={role.id} title={role.title} description={role.description} icon={role.icon} selected={selectedRole === role.id} onClick={() => setSelectedRole(role.id)} />)}
                </div>
              </div>}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {isSignUp && <div>
                  ...
                  <Input id="username" type="text" value={username} onChange={e => setUsername(e.target.value)} placeholder="Choose a username" className="mt-1" required />
                </div>}
              ...
              <div>
                ...
                <Input id="identifier" type={isSignUp ? 'email' : 'text'} value={identifier} onChange={e => setIdentifier(e.target.value)} placeholder={isSignUp ? 'your.email@example.com' : 'Email or username'} className="mt-1" required />
              </div>
            ...
            {!isSignUp && <div className="mt-4 text-center">
                <button type="button" onClick={handleResetPassword} className="text-sm text-primary hover:underline" disabled={resetting}>
            ...
            <div className="mt-4 text-center">
              <button type="button" onClick={() => navigate('/')} className="text-muted-foreground hover:underline text-sm">
                Back to home
              </button>
            </div>
          </Card>
