...
import {
  ...
  connectByStudentCode as connectByStudentCodeApi,
  type ConnectionWithProfile,
} from "@/integrations/api/connections";
import {
  fetchHelpRequests as fetchHelpRequestsApi,
  updateHelpRequestStatus as updateHelpRequestStatusApi,
...
type Connection = ConnectionWithProfile;
...
export default function CaregiverDashboard() {
  ...
  const fetchConnections = useCallback(async () => {
    ...
    try {
      ...
      const enriched = response.data.map((connection) => ({
        ...connection,
        thrive_sprite: connection.thrive_sprite ?? null,
      }));
  ...
  const fetchHelpRequests = useCallback(async () => {
    ...
    try {
      ...
      const activeStudentIds = new Set(connections.filter((c) => c.status === "active").map((c) => c.student_id));
      const filtered = response.data.filter((request) =>
        activeStudentIds.size === 0 ? true : activeStudentIds.has(request.student_id),
      );
  ...
  useEffect(() => {
    ...
    const handleConnectionCreated = (payload: { caregiver_id: string; student_id: string }) => {
      if (payload.caregiver_id === user.id) {
        fetchConnections();
      }
    };

    const handleHelpRequestNew = (request: HelpRequest) => {
      if (!activeIds.has(request.student_id)) return;
      const conn = connections.find((c) => c.student_id === request.student_id);
      const name = conn?.student_profile?.username || "Unknown Student";
      const urgencyVariant =
        request.urgency === "urgent"
          ? "caregiver-urgent"
          : request.urgency === "attention"
            ? "caregiver-warning"
            : "caregiver-success";
      toast({
        title: "New Help Request",
        description: `${getUrgencyEmoji(request.urgency || "ok")} Help request from ${name}`,
    ...
    const handleHelpRequestUpdated = (request: HelpRequest) => {
      if (!activeIds.has(request.student_id)) return;
      fetchHelpRequests();
    };

    socket.on("connection:created", handleConnectionCreated);
    socket.on("help_request:new", handleHelpRequestNew);
    socket.on("help_request:updated", handleHelpRequestUpdated);

    return () => {
      socket.off("connection:created", handleConnectionCreated);
      socket.off("help_request:new", handleHelpRequestNew);
      socket.off("help_request:updated", handleHelpRequestUpdated);
    };
  }, [user?.id, connections, fetchConnections, fetchHelpRequests, playNotificationSound]);
  const handleConnectStudent = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!studentCode.trim()) return;
    setLoading(true);
    try {
      const response = await connectByStudentCodeApi(studentCode.trim().toUpperCase());
      if (response.error || !response.data) {
        toast({
          title: "Erro",
          description: response.error || "Código inválido",
          variant: "destructive",
        });
      } else {
        ...
        toast({
          ...
          description: student
            ? `Conectado com ${student.username} (${student.student_code ?? "sem código"})`
  ...
  const handleHelpRequestAction = async (requestId: string, action: "answered" | "closed") => {
    try {
      const response = await updateHelpRequestStatusApi({ id: requestId, status: action });
      if (response.error) {
  ...
  const handleCloseAllRequests = async () => {
    ...
    try {
      ...
      const responses = await Promise.all(
        openRequestIds.map((id) => updateHelpRequestStatusApi({ id, status: "closed" })),
      );

      const failure = responses.find((res) => res.error);
