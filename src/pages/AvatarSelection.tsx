...
import { upsertMySprite } from '@/integrations/api/thrive-sprites';
...
export default function AvatarSelection() {
  ...
  const getAvatarUrl = (seed: string, style: string) =>
    `https://api.dicebear.com/9.x/${style}/svg?seed=${seed}&size=80&backgroundColor=b6e3f4,c0aede,d1d4f9,ffd5dc,ffdfbf`;
  const avatarStyles = [
    'adventurer',
    'adventurer-neutral',
    'avataaars',
  ...
  const handleSaveAvatar = async () => {
    ...
    setLoading(true);
    try {
      const response = await upsertMySprite({
        imageUrl: getAvatarUrl(selectedSeed, selectedStyle),
  ...
  const handleRandomAvatar = async () => {
    ...
    try {
      const response = await upsertMySprite({
        imageUrl: getAvatarUrl(randomSeed, randomStyle),
