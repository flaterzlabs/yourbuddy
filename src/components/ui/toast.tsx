...
const toastVariants = cva(
  'group pointer-events-auto relative flex w-full flex-col items-center justify-center overflow-hidden rounded-lg border shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-bottom-full data-[state=open]:slide-in-from-bottom-full',
  {
    variants: {
      variant: {
        // Toast do aluno (continua pequeno e discreto)
        student: 'border bg-background text-foreground p-4 text-sm',

        // Toasts do cuidador (desktop + mobile ajustados)
        'caregiver-success':
          'bg-card text-card-foreground border-2 border-emotion-happy shadow-[0_0_30px_hsl(var(--emotion-happy)/0.6)] space-y-2 p-6 max-w-[85vw] md:max-w-[400px] text-base md:text-lg mx-auto',
        'caregiver-warning':
          'bg-card text-card-foreground border-2 border-emotion-need shadow-[0_0_30px_hsl(var(--emotion-need)/0.6)] space-y-2 p-6 max-w-[85vw] md:max-w-[400px] text-base md:text-lg mx-auto',
        'caregiver-urgent':
          'bg-card text-card-foreground border-2 border-emotion-urgent shadow-[0_0_30px_hsl(var(--emotion-urgent)/0.6)] space-y-2 p-6 max-w-[85vw] md:max-w-[400px] text-base md:text-lg mx-auto',
