...
import { connectByCaregiverCode } from '@/integrations/api/connections';
...
export function SettingsModal({ onConnectionAdded, connections = [], trigger }: SettingsModalProps) {
  ...
  const handleConnectCaregiver = async (e: React.FormEvent) => {
    ...
    try {
      const response = await connectByCaregiverCode(caregiverCode.trim().toUpperCase());

      if (response.error || !response.data) {
        toast({
          title: 'Connection failed',
          description: response.error ?? 'Invalid code or caregiver not found',
          variant: "destructive",
        });
      } else {
        const caregiverName = response.data.caregiver?.username ?? 'caregiver';
        toast({
          title: 'Connected successfully!',
          description: `Connected with ${caregiverName}`,
        });
        setCaregiverCode('');
        setOpen(false);
        onConnectionAdded?.();
